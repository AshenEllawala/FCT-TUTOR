...
function emptyInputSignup($email, $studentId, $username, $password) {
    $result=false;
    if (empty($email) || empty($studentId)|| empty($username) || empty($password)) {
        $result = true;
    } else {
        $result = false;
    }
...
function uidExists($conn, $username, $email) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location:student_registration.php?error=stmtfailed");
        exit();
    }
...
// Create the new user in the database
// I've added $studentId and $role to this function
function createStudent($conn, $email, $studentId, $username, $password) {

    $sql = "INSERT INTO student (Email, St_id, Username, Password) 
            VALUES (?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
...
function LoginUser($conn, $username, $password){
    ...
    if($checkpwd===true){
        ...
        $_SESSION["student_id"] = $uidExists["St_id"];
